...
import { ArrowRight, Check, HardDrive, Cpu, Clock, LayoutGrid } from "lucide-react";
...
export default function ConfigPage() {
  ...
  // Handle ID safely (params.id can be string or string[])
  ...
  // Fallback if GPU not found (though existing links should be correct)
  if (!gpu) {
    return (
  ...
  const hourlyRate = useMemo(() => {
    const base = PRICING_BASE[gpuKey] || PRICING_BASE.default;
    // Simple mock formula: Base * Quantity + CPU + Storage
    // In reality this would be more complex
    const cpuCost = cpus * 0.05;
    const storageCost = storage * 0.001;
    return (base * quantity) + cpuCost + storageCost;
  }, [gpuKey, quantity, cpus, storage]);
